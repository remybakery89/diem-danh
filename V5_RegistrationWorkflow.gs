...
function v5EnsureRegistrationStructure_() {
  ...
  for (let i = 0; i < V5_REGISTRATION_HEADERS.length; i++) {
    ...
    if (String(headers[col - 1] || '').trim() !== expected) sheet.getRange(1, col).setValue(expected);
  }
...
function v5GetRegistrationContext_(maBuoi, ma) {
  ...
  for (let i = 1; i < rows.length; i++) {
    if (String(rows[i][1] || '').trim() === targetMaBuoi && normalizeMemberCode_(rows[i][2]) === targetMa) {
      return {
        sheet: sheet,
        rowNumber: i + 1,
        row: rows[i],
...
function v5CountGoing_(sheet, maBuoi) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][1] || '').trim() !== maBuoi) continue;
    const legacy = String(data[i][6] || '').trim().toUpperCase();
    const current = String(data[i][8] || '').trim().toUpperCase();
    if (current === 'ĐI' || (!current && legacy === 'ĐÃ ĐĂNG KÝ')) count++;
  }
  return count;
}
...
function v5SubmitRegistration_(token, maBuoi, requestedStatus, reason) {
  ...
  if (!maBuoi || V5_ALLOWED_STATUS.indexOf(requestedStatus) < 0) return { success:false, type:'INVALID_INPUT', message:'Vui lòng chọn ĐI, KHÔNG ĐI hoặc DỰ BỊ.' };
  ...
  try {
    ...
    if (isFirst && pastDeadline && !reason) return { success:false, type:'REASON_REQUIRED', message:'Đăng ký sau hạn chót, vui lòng nhập lý do.' };
    if (!isFirst && ctx.currentStatus !== V5_CANCELLED_STATUS && ctx.currentStatus !== '' && ctx.currentStatus !== requestedStatus && !reason) return { success:false, type:'REASON_REQUIRED', message:'Thay đổi đăng ký cần có lý do.' };

    if ((isFirst || ctx.currentStatus === V5_CANCELLED_STATUS) && requestedStatus === 'ĐI' && !pastDeadline && program.gioiHan) {
      if (v5CountGoing_(ctx.sheet, maBuoi) >= program.gioiHan) return { success:false, type:'FULL', message:'Chương trình đã đủ số lượng đăng ký.' };
    }
    ...
    const historyEntry = { thoiGian:nowString, loai:isFirst?'ĐĂNG KÝ':'THAY ĐỔI', trangThaiCu:isFirst?'':ctx.currentStatus, trangThaiYeuCau:requestedStatus, trangThaiMoi:currentStatus, lyDo:reason, nguoi:'CA VIÊN' };
    ...
    if (isFirst) {
      ...
      ctx.sheet.getRange(newRow, 9, 1, 7).setValues([[currentStatus, pending ? requestedStatus : '', reason, nowString, '', '', JSON.stringify([historyEntry])]]);
      return { success:true, type:pending?'PENDING':'REGISTERED', message:pending?'Đăng ký đã gửi và đang chờ Admin duyệt.':'Đăng ký thành công.', maDangKy:maDangKy, maBuoi:maBuoi, ma:ma, hoTen:member.hoTen, be:member.be, thoiGian:nowString, trangThai:currentStatus, trangThaiYeuCau:pending?requestedStatus:'' };
    }

    if (ctx.currentStatus === V5_CANCELLED_STATUS || !ctx.currentStatus) {
      ctx.sheet.getRange(ctx.rowNumber, 4, 1, 5).setValues([[member.hoTen, member.be, nowString, 'ĐÃ ĐĂNG KÝ', reason]]);
      ctx.sheet.getRange(ctx.rowNumber, 9, 1, 7).setValues([[currentStatus, pending ? requestedStatus : '', reason, nowString, '', '', JSON.stringify([historyEntry])]]);
      return { success:true, type:pending?'PENDING':'REACTIVATED', message:pending?'Yêu cầu đăng ký lại đã gửi và đang chờ Admin duyệt.':'Đăng ký lại thành công.', maDangKy:String(ctx.row[0]||'').trim(), maBuoi:maBuoi, ma:ma, trangThai:currentStatus, trangThaiYeuCau:pending?requestedStatus:'' };
    }

    ctx.sheet.getRange(ctx.rowNumber, 9, 1, 6).setValues([[currentStatus, requestedStatus, reason, nowString, '', '']]);
    v5AppendHistory_(ctx, historyEntry);
    return { success:true, type:'CHANGE_PENDING', message:'Yêu cầu thay đổi đã gửi và đang chờ Admin duyệt.', maBuoi:maBuoi, ma:ma, trangThai:currentStatus, trangThaiYeuCau:requestedStatus };
...
function v5CancelRegistration_(token, maBuoi, reason) {
  ...
  try {
    ...
    const historyEntry = { thoiGian:nowString, loai:'HỦY', trangThaiCu:ctx.currentStatus, trangThaiYeuCau:V5_CANCELLED_STATUS, trangThaiMoi:V5_CANCELLED_STATUS, lyDo:reason, nguoi:'CA VIÊN' };
...
function v5GetMyRegistrations_(token) {
  ...
  for (let i=1;i<rows.length;i++) {
    ...
    const program=getProgramByCode_(maBuoi);
    if(!program)continue;
    result.push({rowNumber:i+1,maDangKy:String(row[0]||'').trim(),maBuoi:maBuoi,ma:normalizeMemberCode_(row[2]),hoTen:String(row[3]||'').trim(),be:String(row[4]||'').trim(),thoiGian:String(row[5]||'').trim(),trangThaiCu:String(row[6]||'').trim(),trangThai:String(row[8]||'').trim()||(String(row[6]||'').trim()==='ĐÃ ĐĂNG KÝ'?'ĐI':'ĐÃ HỦY'),trangThaiYeuCau:String(row[9]||'').trim(),lyDo:String(row[10]||'').trim(),thoiGianCapNhat:String(row[11]||'').trim(),ngay:Utilities.formatDate(new Date(program.ngay),TIMEZONE,'dd/MM/yyyy'),gio:program.gio?formatSessionTime_(program.ngay,program.gio):'',tenBuoi:program.tenBuoi,diaDiem:program.diaDiem||'',hanChot:program.hanChot?Utilities.formatDate(program.hanChot,TIMEZONE,'dd/MM/yyyy HH:mm'):'',lichSu:v5ParseHistory_(String(row[14]||'').trim())});
  }
...
function v5GetAdminRegistrations_(password, maBuoi) {
  ...
  for(let i=1;i<rows.length;i++){
    ...
    result.push({rowNumber:i+1,maDangKy:String(row[0]||'').trim(),maBuoi:String(row[1]||'').trim(),ma:normalizeMemberCode_(row[2]),hoTen:String(row[3]||'').trim(),be:String(row[4]||'').trim(),thoiGian:String(row[5]||'').trim(),trangThaiCu:String(row[6]||'').trim(),trangThai:String(row[8]||'').trim()||(String(row[6]||'').trim()==='ĐÃ ĐĂNG KÝ'?'ĐI':'ĐÃ HỦY'),trangThaiYeuCau:String(row[9]||'').trim(),lyDo:String(row[10]||'').trim(),thoiGianCapNhat:String(row[11]||'').trim(),nguoiDuyet:String(row[12]||'').trim(),thoiGianDuyet:String(row[13]||'').trim(),lichSu:v5ParseHistory_(String(row[14]||'').trim())});
  }
  const stats={di:0,khongDi:0,duBi:0,choDuyet:0,daHuy:0};
  result.forEach(function(row){if(row.trangThai==='ĐI')stats.di++;else if(row.trangThai==='KHÔNG ĐI')stats.khongDi++;else if(row.trangThai==='DỰ BỊ')stats.duBi++;else if(row.trangThai==='CHỜ DUYỆT')stats.choDuyet++;else if(row.trangThai==='ĐÃ HỦY')stats.daHuy++;});
...
function v5ApproveRegistration_(password, maDangKy, approvedStatus) {
  ...
  try{
    ...
    for(let i=1;i<rows.length;i++){
      ...
      if(current!==V5_PENDING_STATUS||V5_ALLOWED_STATUS.indexOf(requested)<0)return{success:false,type:'NOT_PENDING',message:'Đăng ký này không có yêu cầu chờ duyệt.'};
      if(approvedStatus==='ĐI'){
        const maBuoi=String(rows[i][1]||'').trim();
        const program=getProgramByCode_(maBuoi);
        if(program&&program.gioiHan){
          const count=v5CountGoing_(sheet,maBuoi);
          if(count>=program.gioiHan)return{success:false,type:'FULL',message:'Chương trình đã đủ số lượng đăng ký.'};
        }
      }
      const nowString=Utilities.formatDate(new Date(),TIMEZONE,'dd/MM/yyyy HH:mm:ss');
      const oldReason=String(rows[i][10]||'').trim();
      const history=v5ParseHistory_(String(rows[i][14]||'').trim());
      history.push({thoiGian:nowString,loai:'ADMIN DUYỆT',trangThaiCu:V5_PENDING_STATUS,trangThaiYeuCau:requested,trangThaiMoi:approvedStatus,lyDo:oldReason,nguoi:'ADMIN'});
      sheet.getRange(i+1,9,1,7).setValues([[approvedStatus,'','',nowString,'ADMIN',nowString,JSON.stringify(history)]]);
...
function v5SetupRegistrationWorkflow(){return v5EnsureRegistrationStructure_();}
