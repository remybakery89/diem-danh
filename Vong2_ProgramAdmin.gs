...
function adminSaveProgram_(password,payload){
  ...
  try{
    ...
    if(maBuoiInput){for(let i=1;i<data.length;i++)if(String(data[i][0]||'').trim()===maBuoiInput){rowNumber=i+1;break}if(rowNumber<0)return{success:false,type:'NOT_FOUND',message:'Không tìm thấy mã buổi cần sửa.'}}
    let maBuoi=maBuoiInput;if(!maBuoi){const stamp=Utilities.formatDate(new Date(),TIMEZONE,'yyyyMMddHHmmss');maBuoi='CT'+stamp+Math.floor(100+Math.random()*900);rowNumber=sheet.getLastRow()+1;}
    const oldClose=(maBuoiInput&&rowNumber<=data.length)?data[rowNumber-1][6]:'';
    sheet.getRange(rowNumber,1,1,14).setValues([[maBuoi,new Date(ngayText+'T00:00:00'),new Date('1899-12-30T'+gioText+':00'),tenBuoi,String(payload.diaDiem||'').trim(),moDangKy?new Date('1899-12-30T'+moDangKy+':00'):'',oldClose,gioiHan,String(payload.doiTuong||'').trim(),String(payload.ghiChu||'').trim(),hanChot?new Date(hanChot):'',phuongTien,diemDon,paymentRequired]]);
    sheet.getRange(rowNumber,2).setNumberFormat('dd/MM/yyyy');sheet.getRange(rowNumber,3).setNumberFormat('HH:mm');sheet.getRange(rowNumber,6).setNumberFormat('HH:mm');sheet.getRange(rowNumber,11).setNumberFormat('dd/MM/yyyy HH:mm');
    if(requestedMode)setRegistrationMode_(password,maBuoi,requestedMode);
    return{success:true,message:maBuoiInput?'Đã cập nhật chương trình.':'Đã tạo chương trình mới.',maBuoi:maBuoi,cheDoDangKy:getRegistrationMode_(maBuoi),yeuCauThanhToan:paymentRequired};
  }finally{lock.releaseLock()}
}

function adminGetProgram_(password,maBuoi){
  const auth=verifyAdminPassword(password);if(!auth.success)return auth;const code=String(maBuoi||'').trim();if(!code)return{success:false,message:'Thiếu mã buổi.'};
  const sheet=SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(SHEET_BUOI);if(!sheet)throw new Error('Không tìm thấy sheet BUOI.');v5cEnsurePaymentProgramStructure_();const data=sheet.getDataRange().getValues();
  for(let i=1;i<data.length;i++)if(String(data[i][0]||'').trim()===code){const dateValue=data[i][1],timeValue=data[i][2],openValue=data[i][5],deadlineValue=data[i][10];return{success:true,program:{maBuoi:code,ngay:dateValue instanceof Date?Utilities.formatDate(dateValue,TIMEZONE,'yyyy-MM-dd'):String(dateValue||''),gio:timeValue instanceof Date?Utilities.formatDate(timeValue,TIMEZONE,'HH:mm'):String(timeValue||''),tenBuoi:String(data[i][3]||''),diaDiem:String(data[i][4]||''),moDangKy:openValue instanceof Date?Utilities.formatDate(openValue,TIMEZONE,'HH:mm'):String(openValue||''),hanChot:deadlineValue instanceof Date?Utilities.formatDate(deadlineValue,TIMEZONE,"yyyy-MM-dd'T'HH:mm"):String(deadlineValue||''),gioiHan:String(data[i][7]||''),doiTuong:String(data[i][8]||''),ghiChu:String(data[i][9]||''),phuongTien:String(data[i][11]||''),diemDon:String(data[i][12]||''),cheDoDangKy:getRegistrationMode_(code),yeuCauThanhToan:v5cNormalizePaymentRequired_(data[i][13])}}}
